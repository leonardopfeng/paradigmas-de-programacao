...
void montaSL(double **A, double *b, int n, long long int p, double *x, double *y) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j) {
            A[i][j] = 0.0;
            for (long long int k = 0; k < p - 3; k += 4) {  
                A[i][j] += potenciasX[k][i + j];
                A[i][j] += potenciasX[k + 1][i + j];
                A[i][j] += potenciasX[k + 2][i + j];
                A[i][j] += potenciasX[k + 3][i + j];
            }

            for (long long int k = p - (p % 4); k < p; ++k) {
                A[i][j] += potenciasX[k][i + j];
            }
        }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (long long int k = 0; k < p - 3; k += 4) {  
            ...
            b[i] += potenciasX[k + 1][i] * y[k + 1];
            b[i] += potenciasX[k + 2][i] * y[k + 2];
            b[i] += potenciasX[k + 3][i] * y[k + 3];
        }
