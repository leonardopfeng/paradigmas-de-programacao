...
# Parâmetros de teste
k_pontos_n1 = [64, 128, 200, 256, 512, 600, 800, 1024, 2000, 3000, 4096, 6000, 7000, 10000, 50000, 10**5, 10**6, 10**7]  # K para N1 (grau 10)
k_pontos_n2 = [64, 128, 200, 256, 512, 600, 800, 1024, 2000, 3000, 4096, 6000, 7000, 10000, 50000, 10**5]  # K para N2 (grau 1000)
    ...
    for grupo in grupos:
        ...
        for sub in submetricas:
            ...
            for version, dados in dados_por_versao.items():
                ...
                # Ordena os dados corretamente antes de plotar
                dados_grau_10 = dict(sorted(dados_grau_10.items()))  # Ordena pelo K
                dados_grau_1000 = dict(sorted(dados_grau_1000.items()))  # Ordena pelo K

                tamanhos_10, valores_10 = zip(*dados_grau_10.items()) if dados_grau_10 else ([], [])
                tamanhos_1000, valores_1000 = zip(*dados_grau_1000.items()) if dados_grau_1000 else ([], [])
